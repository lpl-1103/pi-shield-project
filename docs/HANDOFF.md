...
家電控制（獨立的一條，不經過 Mac 也不經過 Aqara 中樞）：

    LINE「關風扇」→ handle_command → ir_remote.py
                                        │ 區網直連
                                        ▼
                                   博聯小黑豆 ──紅外──> 電風扇
```
...
| **紅外線是單向的** | 發出去拿不到任何確認，也讀不到家電目前狀態。機器人回「已送出訊號」只代表訊號離開了小黑豆 |
| **無法分開控制同型號家電** | 同型號＝同一組紅外碼，視線內所有聽得懂的機器會一起動作。要分開只能一台配一個發射器靠遮擋隔開 |
| **Aqara M3 中樞無法被樹莓派控制** | M3 不開放區網介面（§14 實測），官方 Open API 的紅外功能也不含 M3（§16）。唯一路徑是 Matter 場景，上限 15 個 |

## 10. 下一步可以做的

**智慧家電（進行中）**

- **D1 實體按鈕 → M3 內建紅外**：完全在 Aqara App 裡設定，不需要程式。
  做完就有「實體按鈕」與「LINE 指令」兩種控制方式並存
- **口語控制家電**：`nlu.py` 已加入風扇詞彙（§15），
  待 Mac 空閒後重測（當時被驗貨站的訓練佔滿資源）
- **Matter**：M3 唯一能被外部控制的路徑，但只能曝露場景且上限 15 個，
  需要樹莓派跑 Home Assistant。**建議先解決 SD 卡寫入問題**——
  HA 的 recorder 寫入量很大，目前系統跑在 SD 卡上（`/dev/mmcblk0p2`）
...
>
> 保留這一節是為了記錄設計脈絡與可重用的部分（`switch_bridge.py` 的分派、
> 去重、規則檔設計跟訊號來源無關，換 Zigbee dongle 仍可沿用），
> **但不要照著它接線**。目前實際在用的是 §17：樹莓派直接控制小黑豆。

### 目標

按 Aqara D1 雙鍵無線開關 → 中樞收到 → 樹莓派收到 → 博聯小黑豆發紅外 → 家電動作。

### 為什麼需要樹莓派夾在中間

**Aqara 跟博聯是兩個互不相通的生態系。** Aqara 走 Zigbee + 自家雲端，
博聯走 Wi-Fi + 自家雲端，**Aqara App 裡沒有「控制博聯裝置」這個選項**，
兩邊雲端也不互通。D1 → 中樞是原生 Zigbee 沒問題，中樞 → 小黑豆需要翻譯。

    D1 ──Zigbee──> 中樞 ──UDP 多播──> 樹莓派 ──區網──> 小黑豆 ──紅外──> 家電
...
| 檔案 | 做什麼 |
|---|---|
| `src/aqara_hub.py` | 中樞區網協定：`discover()` 用 whois 找中樞、`listen()` 收多播事件 |
| `src/switch_bridge.py` | 橋接常駐：收到事件 → 查規則 → 呼叫 `ir_remote.send()` |
| `deploy/aqara_discover.py` | 設定／診斷工具，即時印出所有事件 |
...
**1. 只「聽」不「控制」，所以不需要金鑰。** 中樞的事件是明文多播，
讀取不需要 App 裡那把 AES 金鑰；要控制中樞上的裝置才需要。
我們只需要知道「按鈕被按了」，因此少一個會過期、會被改動的依賴。

**2. 不預設事件欄位叫什麼。** D1 的 model 字串與按鍵欄位名稱
（`channel_0` / `button_0` / `dual_channel`）**會因批次與韌體而異**。
與其憑猜測寫死，不如讓實際裝置自己講——`aqara_discover.py` 就是為此存在，
規則檔寫什麼欄位就比對什麼欄位。

**3. 規則檔每次事件重讀。** 改規則不用重啟服務，按一下開關就知道對不對。
代價是每個事件多一次檔案讀取，這個頻率下完全不是問題。

### 去重

中樞可能把同一次按鍵送好幾遍。同一組 `(sid, channel, action)` 在 `DEDUPE_SEC`
（預設 1.5 秒）內只執行一次。**這不是可有可無的優化**——多數遙控器的電源是
toggle，被觸發兩次等於沒按。

### ⚠ 尚未對真實硬體驗證

寫的時候中樞還沒插電（使用者缺電源頭）。已完成的驗證：

- 分派邏輯：用假事件測過左右鍵、單擊/雙擊、去重、
...
使用者的中樞是 **Aqara 方舟智慧中樞 M3**（`192.168.0.11`，與樹莓派同網段）。
實測結果：

| 測試 | 結果 |
|---|---|
| ping | ✅ 0% 丟包，同網段確認 |
| whois 區網探索（UDP 4321） | ❌ 無回應 |
| TCP 埠掃描（80/443/8080/9898/4321/5683/5540/8883/1883/54321/22） | ❌ **一個都沒開** |
| mDNS 廣播 | ❌ 沒有任何服務 |

**M3 拿掉了舊的區域網路通訊協定**，改用 Matter/Thread，本機不開任何舊介面。
所以 `aqara_hub.py` 這條路對這台硬體無效——程式本身沒問題，是硬體不支援。

**最終確認（按鈕實測）**：把 D1 無線開關配對到 M3 之後，開 `--raw` 監聽
（連心跳包都印），請使用者實際按左鍵／右鍵／雙擊／長按。
**整段期間收到 0 個封包**。M3 不對區網廣播任何東西，這條路確定不通。

⚠ 這個測試前兩次都是**無效測試**，值得記下來：

1. **第一次**：`nohup python3 ... > log` 沒有 `-u`，stdout 被塊狀緩衝，
   `timeout` 送 SIGTERM 時緩衝區直接丟掉 → 檔案 0 bytes。
   **0 bytes 不等於「沒收到事件」，要先確認程式真的有印出標題。**
2. **第二次**：`pkill -f aqara_discover` **把自己的 shell 也殺了**——
   `-f` 比對完整命令列，而執行這行的 bash 命令列本身就含有這個字串，
   ssh 因此回 255，舊程序沒被殺掉、還抓著已被 `rm` 掉的 inode 繼續寫，
   新檔案永遠是空的。用 `pkill -f '[a]qara_discover'` 中括號技巧可避開。

**教訓**：做「沒收到訊號」這種否定性結論的測試，一定要先有**陽性對照**
——這裡就是「標題有沒有印出來」。沒有對照的話，量測失敗跟結果為零長得一模一樣。

### ✅ 但 M3 內建紅外發射器，使用者的目標有更短的路

App 的中樞頁面有「紅外遙控」項目——**M3 本身就能發紅外**。所以原始目標
（按開關 → 發紅外）**在 Aqara App 裡就能全部完成**：

    D1 開關 ──Zigbee──> M3 ──內建紅外──> 家電

不需要樹莓派、不需要博聯小黑豆，少兩個環節、延遲更低、
也不會因為樹莓派或小黑豆掛掉而失效。**小黑豆只有在家電與 M3 不同房間時才有意義**
（紅外是直線的，要對得到接收窗）。
...
| 做法 | 說明 |
|---|---|
| **Matter** | M3 支援「第三方 Matter 生態」，綁定後樹莓派跑 Matter 控制器可看到子裝置。標準做法，但要多跑一套服務 |
| **Zigbee USB 接收器** | D1 直接跟樹莓派配對，繞過 M3。`switch_bridge.py` 的架構可直接沿用，只要換掉監聽層 |
| **不接** | 按鈕→紅外用 App 就滿足的話，這條可以先不做 |

`switch_bridge.py` / `ir_remote.py` 的分派、去重、規則檔設計**與監聽來源無關**，
之後若改走 Zigbee dongle，換掉 `aqara_hub.py` 那一層即可，其餘不用動。

### ⚠ 博聯 App 裡學過的紅外碼拿不到

使用者已經用手機在博聯 App 裡學過碼，但**那些存在博聯雲端**，
`python-broadlink` 走區網直連讀不到。要用樹莓派**重學一次**
（`ir_remote.learn('名稱')`，拿實體遙控器對著小黑豆按）。
學完存 `~/ir_codes.json`，之後永久有效且完全不碰雲端。
樹莓派上目前**還沒有這個檔案**，代表博聯這條路實際上從沒跑通過。


## 15. 用 LINE 控制家電：不需要經過 M3

### 使用者想要的

> 在 LINE 輸入「關風扇」，openclaw 識別出這是家電操作，風扇就關掉。

### 這條路早就存在，而且不經過中樞

    LINE「關風扇」→ handle_command → ir_remote.send('fan_power') → 小黑豆 → 紅外 → 風扇

`關風扇`／`開風扇`／`風速`／`擺頭` 這些**固定指令本來就寫好了**
（`line_control.py`），跟 M3 完全無關。M3 那條（樹莓派 → M3 → M3 內建紅外）
需要 M3 開放本機控制介面，而它沒有——這跟 §14 測到的「M3 → 樹莓派」是**相反方向**，
但結論一樣：M3 對區網不開任何介面。

**要用 LINE 控制家電，小黑豆才是那條路，M3 幫不上忙。**
M3 的內建紅外只有在 Aqara App 自己的自動化裡才用得到（例如 D1 按鍵 → 紅外）。

### 2026-08-14 補上的：讓口語也能控制家電

原本 `nlu.py` 的指令白名單**沒有家電**，所以「幫我把風扇關掉」不會被翻譯成
`關風扇`，只有打精確指令才有用。已補上：

- 白名單新增 `開風扇` / `關風扇` / `風速` / `擺頭`
- 範例新增「好熱幫我開風扇」「不用吹了」「風太大調小一點」等口語對應
- 加一條規則防止模型把家電當歌名：**「風扇」是家電不是歌名，
  絕對不要輸出「點歌 風扇」**

### ⚠ 這個改動還沒能實測

部署當下 Mac 正在跑驗貨站的模型訓練
（`models/label_counter/train_verifier.py`，15 epochs、PyTorch + 4 個 worker），
load average 6-7、**實體記憶體只剩 94MB**。LM Studio 的 8B 模型搶不到資源，
單一請求超過 120 秒，所有 NLU 呼叫都逾時回 `None`。

**這是資源競爭不是程式錯誤**——連原本正常的「我想聽周杰倫的稻香」也一起逾時。
訓練跑完後要重測一次。

**教訓**：本機 LLM 跟其他吃資源的工作共用一台機器時，
NLU 全部逾時的第一個懷疑對象是「有沒有別的東西在吃資源」，不是程式邏輯。
判斷方法：拿一個**原本就會過的輸入**當對照組，它也失敗就代表是環境問題。

### 目前卡在哪

| 項目 | 狀態 |
|---|---|
| LINE 固定指令 `關風扇` | ✅ 已存在，不需要 openclaw |
| NLU 口語支援 | ✅ 已寫、已部署，⚠ 待 Mac 空閒後實測 |
| 紅外碼 | ❌ `ir_remote.known()` 是空的，一個都沒學 |
| 小黑豆 | ❌ 沒插電，`broadlink.discover()` 找到 0 台 |

**唯一的阻塞是硬體**：小黑豆插電上網 → 用樹莓派學碼 → 整條就通了。
...
## 17. 紅外線遙控實際打通（2026-08-14）

### 裝置

    博聯 RM4C mini（小黑豆）  192.168.0.124  韌體 52079
    跟樹莓派同網段，走區網直連，不經博聯雲端

### ⚠ 坑一：新裝置預設被 App 鎖住，區網一定連不上

`broadlink.discover()` **找得到裝置、認得出型號，但 `auth()` 一律失敗**
（`AuthenticationError: [Errno -1] Authentication failed`）。
連續三次都一樣，單播 `hello()` 直連也一樣。

原因是博聯 App 預設會「鎖定裝置」，擋掉雲端以外的控制。**程式解不開**
（要解鎖得先 `auth()`，但 `auth()` 正是被擋的那一步，是雞生蛋問題）。

**解法只能在 App 裡手動關**：

    博聯 App → 選裝置 → 右上角 ⋯ → 屬性 → 取消勾選「鎖定設備」

關掉之後 `d.is_locked` 變成 `False`，區網直連立刻可用，**且不影響繼續用 App 控制**。

### ⚠ 坑二：博聯 App 裡學過的碼拿不到，一定要重學

那些碼存在博聯雲端，區網直連讀不到。必須用樹莓派重學：

    python3 -c "import ir_remote; ir_remote.learn('fan_power', timeout=120)"

學習模式要有人在現場拿實體遙控器按，沒辦法自動化。實測要點：

- **距離 10-20 公分、正對著**。學習模式需要的訊號強度比平常使用高很多，
  第一次失敗就是距離太遠
- 先用**手機前鏡頭**確認遙控器真的是紅外線（按鍵時看得到紫白色閃光）。
  很多風扇遙控器是 433MHz 射頻，**RM4C mini 完全不支援 RF**
  （`sweep_frequency`/`find_rf_packet` 都是 `False`），那樣永遠學不到
- 輪詢時 `check_data()` 拋 `StorageError` 是**正常的「還沒收到」**，不是錯誤

### 這支遙控器的按鍵配置

**高／中／低三顆同時也是開機鍵，沒有獨立的電源鍵**；另有一顆獨立的關機鍵。
所以 `開風扇` 跟 `大風` 送的是**同一個碼**（實體上就是同一顆鍵）。

最後學到的五個碼：

    fan_high     198 bytes    高速（＝開機）
    fan_medium   214 bytes    中速
    fan_low      210 bytes    低速
    fan_swing    212 bytes    擺頭
    fan_off      316 bytes    關機

因為關機是獨立按鍵，**「關風扇」就是關，不必猜風扇目前狀態**——
多數只有一顆 toggle 電源鍵的遙控器沒有這個好處。

### ⚠ 判斷碼有沒有學完整：看長度是否同一個量級

`fan_high` 第一次學到的是 **80 bytes**，而中／低都是 210 上下。
使用者覺得不對要求重學，重學後變成 198 bytes——**第一次確實是殘缺的碼**。

**同一支遙控器、同一種協定，各按鍵的碼長度應該落在同一個量級。**
某一個明顯偏短就是抓到一半，重學即可。`deploy/test_fan_ir.py` 旁的檢查腳本
用 `< 150 bytes` 當警示門檻。

殘缺的碼**不會報錯**——`learn()` 一樣回 SUCCESS，只是發出去家電沒反應。
這又是一個「執行過程完全正常、只有結果錯」的類型，長度比對是唯一的早期訊號。

`fan_off` 特別長（316）是學碼時按住約一秒錄到重複碼。對關機無害
（關了再關還是關），但若日後行為異常，用輕點一下重學即可。

### ⚠ 坑三：`pkill -f` 自我比對（今天犯了兩次）

第一次是 `pkill -f aqara_discover`，第二次是 `pkill -f '[l]earn'`——
**中括號技巧只保護模式字串本身，但指令裡的 `learn_result.log` 一樣被比中**，
shell 又把自己殺掉，導致學習視窗根本沒開起來，使用者白按了一輪。

**根治做法**：不要在同一行指令裡同時出現 pkill 的模式與其他含該字串的檔名。
這次改成完全不 pkill（先 `pgrep` 確認殘留為 0），並把記錄檔改名成 `irjob.log`。

**而且啟動背景工作後一定要先確認行程真的存在才通知使用者**
（`pgrep -c python3` 加 log 大小），否則使用者按半天是白按的。
這就是 §12 記過的「否定性測試要有陽性對照」，同一天又犯一次。

### 驗證

`deploy/test_fan_ir.py`：組簽章正確的假 LINE webhook 打**真正在跑的服務**，
`開風扇`／`關風扇`／`風速` 三個指令都正確分派並回覆。

⚠ **「有回覆」只代表指令分派正確且紅外訊號已送出，不代表風扇真的收到。**
小黑豆目前還沒擺到風扇前面，最後一哩尚未驗證。紅外是單向的，
發出去收不到任何確認，這個限制永遠存在。

### LINE 指令與說明文件

六個指令全部接上並用真 webhook 對線上服務驗證通過：
`開風扇`／`關風扇`／`大風`／`中風`／`小風`／`擺頭`（`deploy/test_fan_ir.py`）。

**同時重寫了 `help` 與操作手冊**。原本列的燈泡、蜂鳴器、繼電器是 Pi3 Shield 的指令，
**那塊板子早就沒接在樹莓派 4 上**，列著只會誤導使用者。新的 help 分成
點歌／電風扇／其他／說話方式／群組規則五區。

程式裡的 LED 指令**沒有刪掉**——GPIO 跟樹莓派 4 相容，之後要接回來還能用，
只是不再出現在說明裡。

### ✅ 2026-08-14 完整打通（含實體驗證）

小黑豆擺到風扇下方後，**使用者實際觀察風扇動作**確認全部有效：
開機／高速 → 中速 → 低速 → 關機，四段都正確執行。擺頭亦可用。

    D1 那條（實體按鈕）尚未設定；LINE 這條已完全可用：
    LINE「關風扇」→ 樹莓派 → 小黑豆 → 紅外 → 風扇 ✅

### ⚠ 效能問題：每次發送都重新探索，要等 6 秒

`send()` 原本每次都呼叫 `_device()` → `discover()`，那是 **6 秒的 UDP 廣播 timeout**。
使用者在 LINE 傳「關風扇」要**乾等 6 秒**才有反應，連續送多個指令更難用
（而且時序變得不可預期，一度以為是碼有問題）。
...
但 IP 是 DHCP 配發的會變，所以**不能只快取**——`send()` 發送失敗時會帶
`force_rediscover=True` 再試一次，等於「先用快的，壞了才走慢的」，
兼顧速度與正確性。

注意：**每個新的 Python 行程快取都是冷的**。長時間跑著的 Flask 服務會保留快取，
所以只有服務重啟後的第一個指令會慢；用 `python3 -c` 測會每次都看到 6 秒，
那是測試方式造成的，不是實際使用的延遲。

### 搬移裝置不需要重新設定

實測斷電、搬到新位置、重新插電之後：**IP 沒變、碼沒掉、解鎖狀態還在**。

因為紅外碼存在**樹莓派**的 `~/ir_codes.json`，Wi-Fi 設定與解鎖狀態存在**裝置本身**，
而 `ir_remote.py` 用廣播探索不是寫死 IP——三者都不受搬移影響。

（過程中有一次找不到裝置，原因是**插座沒開電源**，不是網路問題。
排查順序建議：先確認通電 → 再看裝置的燈 → 最後才查網路。）

### 硬體特性（影響擺位與擴充）

- **距離約 8 公尺、360° 全向發射**，不必對準，放房間中央效果最好
- 紅外**會反射**，小房間裡打天花板再彈到家電通常也有效
- **不能穿牆**
- **無法分開控制多台同型號家電**——同型號＝同一組碼，
  視線範圍內所有聽得懂的機器會一起動作。要分開只能一台配一個發射器靠遮擋隔開
