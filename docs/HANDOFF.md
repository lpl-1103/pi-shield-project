...
學完比對兩個碼：
    ...
    fan_power     80 bytes
    fan_off      316 bytes    內容完全不同

所以 `關風扇` 改送 `fan_off` 而不是跟開機共用 `fan_power`。
好處是「關」就是關，**不必猜風扇目前是開還是關**——
多數只有一顆 toggle 鍵的遙控器沒有這個好處。

`fan_off` 特別長很可能是學碼時按住約一秒，錄到了紅外的重複碼。
對「關」來說無害（關了再關還是關），但若日後行為異常，用輕點一下重學即可。
...
- 小黑豆擺位到能照到風扇，做真正的端對端驗證
- `fan_speed` / `fan_swing` 兩個碼還沒學（`風速`／`擺頭` 目前會回「還沒學過」）
