...
完整指令、按鍵對照、API 說明見 [`docs/pi3_control.md`](docs/pi3_control.md)。

## 📚 文件

| 文件 | 內容 |
|---|---|
| [`HANDOFF.md`](docs/HANDOFF.md) | 完整開發過程紀錄——每個階段做了什麼、踩過哪些坑、為什麼這樣設計，按時間順序寫的第一手記錄 |
| [`docs/pi3_control.md`](docs/pi3_control.md) | 操作手冊：按鍵對照表、LINE 指令列表、Web API 說明 |
...
```
src/pi3_control.py     LED / 蜂鳴器 / 繼電器核心邏輯 + 終端機按鍵互動介面
src/line_control.py    Flask app：LINE Webhook、網頁面板、點歌系統路由
src/karaoke.py         點歌佇列引擎：播放迴圈、mpv IPC、歌詞抓取、熱門歌曲電台
src/nlu.py             自然語言翻譯層（呼叫 Mac 上的 openclaw / 本機模型）
src/stt.py             語音轉文字（呼叫 Mac 上的 mlx-whisper 服務）

docs/pi3_control.md    操作手冊：按鍵對照、LINE 指令、Web API
docs/HANDOFF.md        交接文件
pi3_basic/             Pi3 Shield 的 C 語言基礎範例（bcm2835）
```

> 📁 2026-08-07 目錄重整：程式移到 `src/`、文件移到 `docs/`。
> **樹莓派上的部署路徑沒有變**（那邊是 `~/pi3_control.py` 之類的獨立副本），
> 這次只動本機這份 repo 的擺放方式。
