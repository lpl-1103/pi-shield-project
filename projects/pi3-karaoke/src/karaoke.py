...
# ---------- 熱門歌曲清單 ----------
#
# ⚠ 這份寫死的清單**只剩備援用途**。每類只有 12 首，約 48 分鐘就會繞回來，
# 而使用者要的是連續播 4 小時（約 60 首）不重複。正式的選歌走 radio_pool，
# 它用種子關鍵字動態抓，池子會累積到數百首。
# 只有在 radio_pool 完全抓不到東西時（沒網路之類）才會退回用這份。
POPULAR_SONGS = {
    'kpop': [
...
def _record_history(song, video_id, url):
    # 長期統計另外存一份（SQLite，永久）。**電台自動播的不記**——
    # 電台一小時放十幾首，記進去會淹沒真正的點播紀錄，
    # 「這個人常點什麼」就失去意義了。
    if not str(song.requester or '').startswith('🔀'):
        try:
            song_stats.record(song.requester, song.title, song.query,
                              video_id, song.mode, normalize=_normalize_title)
        except Exception:
            pass          # 統計壞掉絕不能影響播放
...
def _pick_radio_song(category):
    ...
    優先走 radio_pool 的動態池（數百首，且有「這次播過」的硬性排除），
    抓不到才退回寫死的 POPULAR_SONGS——那只會發生在沒網路的時候。

    回傳的是可以直接餵給 yt-dlp 的字串：動態池回傳 YouTube 網址（精確），
    備援清單回傳搜尋關鍵字。
    """
    song = radio_pool.pick(
        category,
        exclude_ids=get_played_video_ids(),
        exclude_title_keys=get_played_title_keys(),
...
def start_radio(category):
    ...
    # 先在背景抓一批，讓第一首不用等
    radio_pool.warmup(category)
    return True


def stop_radio():
    """停止電台。**這裡才清「這次播過」的紀錄**——使用者說停了之後
    下次開始有少部分重複沒關係，所以只在停止時重置，播放中絕不重置。"""
    global _radio_category
    with _lock:
        _radio_category = None
    radio_pool.reset_session()
    skip()
