...
MENU_TEXT = (
    ...
    "  常點 / 熱門排行     = 你最常點的歌 / 全場最常被點的歌，回數字直接點\n"\
    "  大螢幕            = 傳送接電視用的大字歌詞頁面連結\n"\
    "  面板              = 傳送圖形控制面板連結\n"\
    "\n"\
    "🌀 電風扇（紅外線遙控）\n"\
    "  開風扇 / 關風扇     = 開機 / 關機\n"\
    "  大風 / 中風 / 小風   = 直接切到高速 / 中速 / 低速\n"\
    "                       （遙控器的高中低就是開機鍵，所以「開風扇」＝「大風」）\n"\
    "  擺頭              = 左右轉向開關\n"\
    "\n"\
    "🌤 其他\n"\
    "  天氣 / 氣溫        = 查目前天氣（新北市三重）\n"\
...
<body>
  ...
  <h2>🌀 電風扇（紅外線遙控）</h2>
  <div class="card">
    <code>開風扇</code> / <code>關風扇</code><br />
    <code>大風</code> / <code>中風</code> / <code>小風</code> — 直接切到該風速<br />
    <code>擺頭</code> — 左右轉向開關<br />
    <span class="sub">
    這支遙控器的高／中／低三顆<strong>同時也是開機鍵</strong>，沒有獨立的電源鍵，
    所以「開風扇」跟「大風」是同一顆。關機則有獨立按鍵，
    因此「關風扇」就是關，不必猜風扇目前是開還是關。<br />
    訊號由樹莓派透過博聯小黑豆發出，走區網直連不經雲端。
    <strong>紅外線是單向的</strong>——機器人回「已送出訊號」只代表訊號發出去了，
    小黑豆必須對得到風扇的接收窗，家電才會有反應。</span>
  </div>
...
def handle_command(text: str, base_url: str = '', user_id: str = None) -> str:
    ...
    # ---- 風扇（紅外線）----
    # 這支遙控器的按鍵配置（2026-08-14 學碼時確認）：
    #   高 / 中 / 低 三顆 —— **同時也是開機鍵**，按下去就是「開機並切到該風速」
    #   關 一顆        —— 獨立的關機鍵
    # 所以「開風扇」跟「大風」送的是同一個碼（fan_high），因為實體上就是同一顆鍵。
    # 沒有獨立的電源鍵，也因此不存在「按一下開、再按一下關」的 toggle 問題——
    # 「關風扇」就是關，不必猜風扇目前是開還是關。
    if key in ('開風扇', '風扇開', '打開風扇', '開電風扇'):
        return ir_remote.send('fan_high', '風扇')
    if key in ('關風扇', '風扇關', '關掉風扇', '關電風扇'):
        return ir_remote.send('fan_off', '風扇')
    if key in ('大風', '強風', '高速', '高風', '風扇高速', '最大風'):
        return ir_remote.send('fan_high', '高速')
    if key in ('中風', '中速', '中等風', '風扇中速'):
        return ir_remote.send('fan_medium', '中速')
    if key in ('小風', '微風', '低速', '低風', '風扇低速'):
        return ir_remote.send('fan_low', '低速')
