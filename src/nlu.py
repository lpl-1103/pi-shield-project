...
特別注意「聽到X」這個模式：在點歌情境下，「我想聽到X」幾乎都是
「我想聽」＋歌名的誤判（「到」是把歌名第一個字聽錯了），不要把「到」當成動詞
「聽到」的一部分而只留下 X。例如「我想聽到香」是「我想聽稻香」不是「聽到」＋「香」。
如果聽起來像某首知名歌曲的歌名，請自動更正成正確的歌名再輸出。例如：
道香 / 到香 / 到響 / 到聲 / 到聲音 / 到香了 -> 稻香
（所以「我想聽到香」「我要聽到聲音」都是「點歌 稻香」）
告白汽球 / 告白氣求 -> 告白氣球
晴天娃娃（在點歌情境下）-> 晴天
溫柔鄉 -> 溫柔
...
「風扇」是家電不是歌名——講到開關風扇、冷熱、風速、擺頭一律輸出家電指令，
絕對不要輸出「點歌 風扇」這種東西。
...
_TIMEOUT = 45
# 實測延遲很不穩：命中 LM Studio 的提示詞快取約 3~5 秒，沒命中要重新處理
# 整段系統提示詞就會拖到 20 秒以上。25 秒的話大約每 4 次會逾時 1 次
# （使用者就看到「不認識的指令」，但其實只是還沒算完）。
# 語音流程本來就是關掉麥克風後等結果，寧可多等也不要白白失敗。
